...
Statistics is the practice concerned with collecting and analyzing data. The
interactive document is not strictly about statistics but rather, about doing
the basic statistical summarizing and testing in R. We will only be looking at
a few ideas in statistics that will be needed for the upcoming modules. Namely
...
three variables and collect them in a `data.frame`. You can think of this object
`d` as encoding an experimental data. In the following sense
...
retrieve it using statistical tools. Here is a code that adds 1 to the 
measurements of the group `'B'`, and leaves `'A'` as is.
...
Use `str`, `head` or `tail` to check whether the code above produced what it 
aims to do. In the next section, we will use summary statistics to explore the
bias we introduce between the groups using different statistical and visual 
tools.

## Summary statistics 

Summarizing data is a sensible starting point when dealing with many data points
. Two useful summary statistics are averages and variances.
...
There are in R functions that correspond to the first two of these statistics. 
These are `mean` and `median`.

Here is the code to calculate the mean for all measurements and for each of the
two groups in the data A and B.
...
Now try to answer these questions based on what you've done.
...
These single numbers tell us something about the center of the data point. Other
statistics can tell us about how much the data points vary from each other. We 
turn to this next.
...
Here is the code to calculate the variance and the stardard deviation for all 
measurements
...
Now try to calculate the these same statistics for the groups A and B.
...
ways to get a feel of how the data itself look like. One way is to use graphs
to explore the data. We will be discussing some of the graphing (plotting) 
tools in the next section.

## Plots 

There are many different to visualize any set of data. The basic graphs such as
bar, line and scatter plots are indispensable. Used in the right way and context
they can be very illuminating and once your data is large enough, graphs like 
these will be the only way to look into the data and the relationships between
the variables.
...
The only required argument you need to pass to `barplot` is called `height`, the
rest is optional (to make the plot look pretty). Here is an example
...
Remember, you can can calculate the means just like before and save the results
...
The code below shows some embellishment in the above graph, just to show you how
...
Here is code to to plot all three variables in the object `d2`, which we created
earlier. The code puts `V1` on the x-axis, `V2` on the y-axis and color by `V3`.
...
Modify the code below to change the x to 'Variable 1' and y to 'Variable 2'. 
You will need to pass these labels to the arguments `xlab` and `ylab`.
...
word, it divides the data into ranges called bins and count how many occur in
each of these bins
...
to make a tiny modification of the code.
...
Looking at the mean, variance, and the plots we made, you can easily till that 
the bias we introduced in the group 'B' is there. Now, we introduced this 
difference intentionally. But in most cases, this would be the exact question we
as of the data. Is there a difference between the measurements in group A to B.
...
whether the mean anything is beyond the topic at hand.
...
Check the help page of the `t.test` function and try to interpret the output of
the test above.
