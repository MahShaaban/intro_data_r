...
There are a few things you need to know to get started using R. These are 
...
can sum and count items in a variable.
...
takes as an input a variable `x` and computes its mean. It does it by dividing
...
they make life easier for you and your computer, but at the cost of a bit of 
...
The next section will take you through one way to have tables with different 
types of data in them. These are called "Data frames".

## Data frames 

`data.frame`s are basically tables. The consist of rows and columns. Columns can
...
The function `data.frame`, takes as input the columns and returns, well, a 
`data.frame`.
--
...
That's it about `data.frame`s for now. We will be using them in again soon. But 
