...
Quantitative real-time PCR is an important technique in medicine and bio-medical
...
In a real time PCR assay, a reaction is quantified by detecting fluorescent 
signal. The signals are reported as $C_T$ values (cycle threshold). $C_T$ levels
are inversely proportional to the amount of template in the reaction.
...
function that implement the $\Delta\Delta C_T$ model and more.

## Analyzing $C_T$ values

In this section, we will be using the `pcr` package to analyze $C_T$ values of
C-Myc (MYC) in brain and kidney tissues. The goal is to estimate the relative 
expression of the gene these tissues.
