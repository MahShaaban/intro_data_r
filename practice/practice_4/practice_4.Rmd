...
We will be using several functions from `colocr` to apply the workflow  step by 
step.
...
this experiment, the cell line was stained with probes for two proteins RKIP and
...
questions. The functions `dim` and `unique` can be helpful.
...
The word 'channel' here refer to the colors red, blue, and green. In this image, 
RKIP was labeled red, LC3 green and the nucleus blue. Channels 1, 2 and 3
...
documented in the function's help page `?roi_select`.
...
Now, to make sure these parameters are appropriately encompassing the ROI, call
...
2. There is an argument, `ind`, which requires the integers corresponding to the 
...
To make life easier, the package provides a simple shiny app to interactively 
determine these parameters and use it in the rest of the workflow. 
...
the first argument, and the type of co-localization statistics second.
