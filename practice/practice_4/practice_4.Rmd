...
A few R packages are available for conducting image analysis, which is a very
wide topic. As a result, some of us might feel at a loss when all they want to
do is a simple co-localization calculations on a small number of microscopy
images. 
...
Included in the package, is a shiny app that can be invoked locally to 
...
We will be using several functions from `colocr` to run it step by step.
...
this experiment, the cell line was treated with probes for two proteins RKIP and
LC3. The aim of this experiment is to determine, how much of the two proteins 
are co-localized or co-distributed in the particular cell line.

We will first learn how to load these images in R and look inside the object to
...
The package provide a convenient functions to load images in R. It's called 
...
Print the object image and try to answer the following questions about it.
...
`data.frame` and have a look at it. There is a few ways to do this. Use the 
...
dimensions and the unique values in each column to help you answer these
questions. The functions `dim` and `unique` should be helpful.
...
The word channel here refer to the colors red and green. In this particular
...
the first two colors separately.
...
amount of co-localization. This workflow consists of two steps only:
...
We will examine each of these functions in turn in the coming topics

## Selecting regions of interest 

Typically, one wants to select the regions of the image occupied by a cell or a 
group of cells. The package can also be used to select certain areas/structures 
...
background knowledge in the workings of the algorithms and can be done through 
...
documented in the function help page `?roi_select`.
...
This function returns `cimg` object containing the original input image and an 
...
Now, to make sure these parameters are appropriately encompassing the ROIs, call
...
To make life easier, the package provides a simple shiny app to interactively determine these parameters and use it in the rest of the workflow. 
...
And here is a screen shot from the app after applying the same parameters used
previously.
...
and can run the same analysis described here. The app can be accessed from the 
web [here](https://mahshaaban.shinyapps.io/colocr_app2/).

We won't be discussing the app any further, but you can always click the link
and check it for yourself.

Next we turn to the main purpose of the package, with is calculating the
co-localization statistics between two channels.

## Calculating Correlation Statistics 

Let's start by discussing two of the co-localization statistics that are 
...
where $R_i$ and the $G_i$ is the intensities of the red and green channels and 
...
of interest and in the practice you will be working with more than one images.
...
the image with regions of interests highlighted.
